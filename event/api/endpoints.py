...
from event.dto.request import (
    CreateEventRequest,
    EventRegistrationRequest,
    UpdateEventRequestDto,
    convert_create_event_request_to_event_dto,
)
from event.dto.response import (
    EventResponse,
    CreateEventResponse,
    UpdateEventResponse,
    DeleteEventResponse,
...
def create_new_event_api(request: Request, payload: CreateEventRequest, db: Session = Depends(get_db)):
    try:
        ...
        return CreateEventResponse(
            status="Success", message="Event Created Successfully", data=EventResponse(event=event_dto)
        )
...
def update_event_api(request: Request, event_id: int, payload: UpdateEventRequestDto, db: Session = Depends(get_db)):
    ...
    try:
        ...
        return UpdateEventResponse(
            status="Success", message="Event Updated Successfully", data=EventResponse(event=updated_event)
        )
    except HTTPException as err:
        raise
    except Exception as err:
...
def event_registration_api(event_id: int, payload: EventRegistrationRequest, db: Session = Depends(get_db)):
    ...
    try:
        ...
        return EventRegistrationResponse(
            status="success", message="Registration successful", data=new_event_registration
        )
    except HTTPException as err:
        raise
    except Exception as err:
